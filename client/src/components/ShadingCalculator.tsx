...
export default function ShadingCalculator({ initialPoints, templateData, weatherData, onPointsChange, onWeatherDataOverride, onFacadeAnalysis3D, onModelDataReady, externalActiveFacadeIdx }: { initialPoints?: any[] | null; templateData?: string[][] | null; weatherData?: EPWData | null; onPointsChange?: (points: Array<{month: string; day: number; hour: number; solarHeight: number; solarAzimuth: number; obstacle: string; shadedArea: number; fs: number; fsGeometrico?: number; fsClimatico?: number; fsCombinado?: number}>) => void; onWeatherDataOverride?: (data: EPWData) => void; onFacadeAnalysis3D?: (analysis: FacadeFullAnalysis | null) => void; onModelDataReady?: (data: { facades: DetectedFacade[]; obstacleVertices3D: Vertex3D[][] | undefined; northOffset: number }) => void; externalActiveFacadeIdx?: number | null }) {
      ...
      try {
        if (isNewFormat && detectedFmt) {
          ...
          try {
            ...
            const result = convertOBJToObstacles(parsed, evaluationModel?.groundLevel, northOff, objSwapYZ, objScale, { x: objOffsetX, y: objOffsetY, z: objOffsetZ }, evaluationModel?.config.rotationDeg ?? 0);
  ...
  const reconvertOBJ = useCallback(() => {
    ...
    const northOff = sunPath3DPreview?.location.northOffset ?? objNorthOffset;
    const result = convertOBJToObstacles(objRawParse, evaluationModel?.groundLevel, northOff, objSwapYZ, objScale, { x: objOffsetX, y: objOffsetY, z: objOffsetZ }, evaluationModel?.config.rotationDeg ?? 0);
    setObjPreview(result);
              ...
              <label className="flex items-center gap-2 text-xs">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (objRawParse) {
                      const northOff = sunPath3DPreview?.location.northOffset ?? objNorthOffset;
                      const result = convertOBJToObstacles(objRawParse, evaluationModel?.groundLevel, northOff, e.target.checked, objScale, { x: objOffsetX, y: objOffsetY, z: objOffsetZ }, evaluationModel?.config.rotationDeg ?? 0);
                      setObjPreview(result);
                    }
                  }}
              ...
              <label className="flex items-center gap-2 text-xs">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (objRawParse) {
                      const northOff = sunPath3DPreview?.location.northOffset ?? objNorthOffset;
                      const result = convertOBJToObstacles(objRawParse, evaluationModel?.groundLevel, northOff, objSwapYZ, newScale, { x: objOffsetX, y: objOffsetY, z: objOffsetZ }, evaluationModel?.config.rotationDeg ?? 0);
                      setObjPreview(result);
                    }
                  }}
              ...
              <label className="flex items-center gap-2 text-xs">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (objRawParse) {
                      const result = convertOBJToObstacles(objRawParse, evaluationModel?.groundLevel, newOffset, objSwapYZ, objScale, { x: objOffsetX, y: objOffsetY, z: objOffsetZ }, evaluationModel?.config.rotationDeg ?? 0);
                      setObjPreview(result);
                    }
                  }}
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (objRawParse) {
                          ...
                          const result = convertOBJToObstacles(objRawParse, evaluationModel?.groundLevel, northOff, objSwapYZ, objScale, nextOffset, evaluationModel?.config.rotationDeg ?? 0);
                          setObjPreview(result);
                        }
                      }}
                      className="w-16 px-2 py-1 border border-gray-300 rounded text-xs font-mono bg-white"
