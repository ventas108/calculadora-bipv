/**
 * Renombrado desde server/pvgisProxy.test.ts (CodeSpecs/02-recurso-solar/react,
 * tarea #6): este archivo nunca probó server/pvgisProxy.ts -- prueba
 * client/src/lib/irradianceHeatmap.ts, una librería de estimación SINTÉTICA
 * de irradiancia por latitud/longitud (sin llamar a PVGIS).
 *
 * Verificado (16-sep-2026): irradianceHeatmap.ts no tiene ningún consumidor
 * real en el resto del repo -- el componente real (IrradianceHeatmap.tsx)
 * consulta PVGIS de verdad vía /api/pvgis, no esta librería. Es código muerto
 * hoy. Se conserva este archivo (solo renombrado, contenido sin cambios) en
 * vez de borrarlo, por prudencia -- decisión de borrar irradianceHeatmap.ts
 * queda pendiente para el usuario/Copilot, fuera del alcance de esta tarea.
 * La cobertura real de server/pvgisProxy.ts vive ahora en
...
describe("irradianceHeatmap library (sintética, sin consumidor real -- ver nota arriba)", () => {
