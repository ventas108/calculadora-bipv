...
from calculos.presupuesto import (
    BENCH as _BENCH,
    ZONA_FACTOR as _ZONA_FACTOR,
    calcular_parametrico as _calc_parametrico,
    recolectar_items_cotizacion as _recolectar_items_cotizacion_core,
)
...
# La lógica de agregación vive en calculos/presupuesto.py — aquí solo se
# extraen las secciones de session_state (columna Activo, Cantidad, USD_un)
# y se le pasan al módulo, que no toca Streamlit.
_secciones_cot = {
    _key: st.session_state.get(f"df_sec_{_key}")
    for _key in ("perfileria", "mano_obra", "sistema_fv", "inversor", "catalogo")
}
_items_cot = _recolectar_items_cotizacion_core(_secciones_cot, tc, _SEC_LABELS)
