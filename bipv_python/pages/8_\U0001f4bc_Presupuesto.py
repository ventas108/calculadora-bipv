...
with t5:
    st.markdown("**Equipos del catálogo — se sincronizan con 📐 Dimensionamiento**")
    cat_rows = []
    if n_pan > 0:
...
m2.metric("Indirectos + Impr.",  f"USD {indirectos+imprevistos:,.0f}", f"$ {(indirectos+imprevistos)*tc/1e6:.2f} M COP", delta_color="off")
...
st.success(f"✅ CAPEX **USD {capex_total:,.0f}** ($ {capex_total*tc/1e6:.2f} M COP) guardado → 💰 Financiero lo usa automáticamente.")
