"""
Prueba manual del Analista Técnico-Financiero — CON COSTO REAL DE API.
...
Uso:
    python scripts/probar_analista_tecnico_financiero.py

Qué hace:
1. Corre sensibilidad + generación de candidatos + Pareto (Fase 4) sobre
   un TMY sintético offline (Bogotá) — cero costo, ya validado en
   tests/test_optimization_fase4.py.
2. Re-simula el frente de Pareto para tener el FinancialResult completo
   de cada candidato (la Fase 4 solo guarda los objetivos extraídos).
3. Llama al Analista Técnico-Financiero UNA vez sobre esos resultados
   reales — aquí es donde se gasta dinero.
"""
import dataclasses
import os
import sys
...
from optimization.sensitivity import analizar_sensibilidad
...
from agentes.analista_tecnico_financiero import (
    CandidatoRegistrado, ejecutar_analisis, texto_final,
)
...
def main() -> None:
    ...
    print("2/4 — Sensibilidad + generación de candidatos + Pareto (sin costo, motor determinista)...")
    variables_geo = opt_vars.variables_geometria("Fachada")
    resultados_sens = analizar_sensibilidad(
        cfg_base, variables_geo + [opt_vars.variable_k_bipv()], tmy,
        fin_config_builder=_fin_builder,
    )
    candidatos = generar_candidatos(cfg_base, variables_geo, n_candidatos=10, seed=42)
    ...
    print("4/4 — Llamando al Analista Técnico-Financiero (claude-opus-5 — ESTO GASTA DINERO)...\n")
    mensaje = ejecutar_analisis(resultados_sens, registro)
