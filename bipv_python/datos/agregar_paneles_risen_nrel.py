# -*- coding: utf-8 -*-
"""
Script de uso unico: agrega paneles reales de Risen Energy (Mono-Si) al
catalogo real (datos/paneles_catalogo.xlsx::Catalogo_Paneles_FV) -- ver
DIAGNOSTICO_CATALOGO_RISEN_NREL.md para el detalle completo.

Mismo patron que JA Solar/Trina/Jinko/Canadian Solar/LONGi: exclusion
por tolerancia SDM (validar_sdm_vs_ficha, 6%). 11/148 excluidos -- mismo
patron half-cut ya documentado en Trina/Jinko (V/celda~0.33-0.34V, la
mitad de lo real, familias RSM120-6/RSM144-6: 120/144 son conteos
fisicos de medias-celdas, no la profundidad electrica real de 60/72).

Elegido por posicionamiento real confirmado en el mercado colombiano
(492 MW suministrados a la planta Guayepo I y II; distribuidores locales
Energía Solar Colombia y Ferragro -- verificado via busqueda web el
3-sep-2026, a diferencia de otros candidatos del mismo volumen como
Boviet Solar, Phono Solar o Hyundai Energy Solutions, sin presencia
...
Uso:
    cd bipv_python
    python datos/agregar_paneles_risen_nrel.py
"""
import csv
import re
import sys
...
def _construir_pares_risen(pvs, cec):
    pvs_norm = {}
    for n in pvs:
    ...
    return [
        ...
        for k in comunes
        if cec[cec_norm[k]]["Manufacturer"].strip().lower() == "risen energy co ltd"
    ]
...
def _construir_fila(nombre_cec, pv, c):
    modelo = nombre_cec.replace("Risen Energy Co Ltd ", "").strip()
    Voc, Vmp, Isc, Imp = (float(c["V_oc_ref"]), float(c["V_mp_ref"]),
                          float(c["I_sc_ref"]), float(c["I_mp_ref"]))
    Pmax = float(c["STC"])
    Ns = int(c["N_s"])
    ...
    partes = [
        ...
        "Marca con presencia real confirmada en Colombia (492 MW suministrados "
        "a planta Guayepo I y II; distribuidores locales Energía Solar Colombia "
        "y Ferragro).",
    ]
    if not tiene_dim:
        partes.append(
    ...
    return {
        "ID": None, "Marca": "Risen Energy", "TipoPanel": modelo,
...
def main():
    ...
    pares = _construir_pares_risen(pvs, cec)
    print(f"Candidatos Risen Energy (match normalizado): {len(pares)}")
