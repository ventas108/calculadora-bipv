# -*- coding: utf-8 -*-
"""
Script de uso unico: agrega paneles reales de LONGi al catalogo real
(datos/paneles_catalogo.xlsx::Catalogo_Paneles_FV) -- ver
DIAGNOSTICO_CATALOGO_LONGI_NREL.md para el detalle completo.

Mismo patron que agregar_paneles_canadian_nrel.py. Cruce por nombre
NORMALIZADO (sin puntuacion) entre las 2 fuentes reales, mismo umbral de
auditoria fisica real (6%, validar_sdm_vs_ficha).

Nombre legal real del fabricante en ambas fuentes: "LONGi Green Energy
Technology Co Ltd".

Auditoria real: de 244 candidatos con match normalizado, **0 fallan la
tolerancia real de produccion (6%)** -- el unico de los 5 fabricantes
importados con cero exclusiones electricas. A cambio, la cobertura de
dimensiones es la mas baja hasta ahora: 209/244 (86%) sin Length/Width en
CEC, solo area total -- verificado real en la fuente, no un error de este
script (consistente con el 14% de cobertura ya visto en el sondeo
inicial de mercado). Quedan 244 importados (todos).

Fuentes (mismas 2 que los 4 imports anteriores, ver esos scripts para
las URLs completas):
...
Con este import quedan cubiertos los 5 fabricantes Tier 1 confirmados
como dominantes en el mercado colombiano (Jinko, LONGi, Trina, Canadian
Solar, JA Solar).

Uso:
    cd bipv_python
    python datos/agregar_paneles_longi_nrel.py
"""
import csv
import re
import sys
...
def _construir_pares_longi(pvs, cec):
    """Cruce normalizado, solo LONGi, primer match por clave normalizada."""
    pvs_norm = {}
    for n in pvs:
    ...
    return [
        ...
        for k in comunes
        if cec[cec_norm[k]]["Manufacturer"].strip().lower() == "longi green energy technology co ltd"
    ]
...
def _construir_fila(nombre_cec, pv, c):
    ...
    modelo = nombre_cec.replace("LONGi Green Energy Technology Co Ltd ", "").strip()
    ...
    partes.append(
        ...
        "real (ver DIAGNOSTICO_CATALOGO_LONGI_NREL.md)."
    )

    return {
        "ID": None, "Marca": "LONGi", "TipoPanel": modelo,
...
def main():
    ...
    pares = _construir_pares_longi(pvs, cec)
    print(f"Candidatos LONGi (match normalizado): {len(pares)}")
