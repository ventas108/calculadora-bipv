...
**Sugerencia orientativa del total desde el área disponible (29-ago-2026)**: probando un inversor pequeño (SNA-3K, clase residencial/híbrida) el usuario notó que "N total de cadenas para el proyecto" no tenía NINGUNA orientación — ni el área, ni ningún otro dato del proyecto sugería un valor, había que escribirlo a ciegas. Agregado: un cálculo informativo (`área útil ÷ área del panel ÷ N/string`, usando el N ya confirmado si existe o si no una estimación eléctrica mínima) que se muestra como caption y en el tooltip del campo — **no se autocompleta, es solo una referencia** para no partir de cero. No considera un layout físico real de paneles en la fachada, solo área bruta disponible.
...
Novedades de esta versión: 📐 Dimensionamiento — "N total de cadenas para el proyecto" no tenía NINGUNA orientación (ni el área, ni ningún otro dato del proyecto sugería un valor) hasta que el usuario, probando un inversor pequeño (SNA-3K), preguntó explícitamente si algo debía guiar ese número. Agregado un cálculo informativo (área útil ÷ área del panel ÷ N/string ya confirmado o estimado eléctricamente) mostrado como caption y tooltip -- no se autocompleta, es solo referencia, no un layout físico real. Ver sección 6.
