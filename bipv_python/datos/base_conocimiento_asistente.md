...
Novedades de esta versión: `optimization.variable_panel()` conectado al catálogo Excel real de 65 paneles (antes solo 7) para que el optimizador de Fase 4 y cualquier agente puedan barrer panel real × geometría × inversor al buscar mejor TIR — 4 bugs reales encontrados y corregidos en la misma auditoría (3 apariciones del mismo patrón de `KeyError` por resolver contra el catálogo viejo, 1 `TypeError` real en el modelo simplificado de producción). Ver sección 24.3.
