...
### Auditoría real con 3 fichas MUST (PV3500 TLV Series) + hallazgo crítico de archivo mal nombrado (30-ago-2026)
...
Novedades de esta versión: 🔌 Catálogo de Inversores PDF — auditoría real con las 3 fichas MUST PV3500 TLV Series pedidas por el usuario. Hallazgo crítico: el archivo del escritorio nombrado como "PV35-12048 TLV" en realidad contiene la ficha de la serie PV3600 (PV36-8048/10048/12048, MPPT muy distinto) — no un bug de código, un archivo mal nombrado; ya renombrado en el escritorio del usuario para que no se cargue por error. 2 bugs reales de código corregidos: `P_dc_max_W` no se extraía por un typo real del fabricante ("Maximim" en vez de "Maximum"), y el campo `modelo` confundía el encabezado de sección "INVERTER" con un código de modelo real (fix general, no solo MUST). Primer test directo del motor de extracción de inversores (`tests/test_pdf_inversor_extractor.py`, 6 tests) — antes no tenía ninguno. Suite completa: **767/767**. Ver sección 13b y `DIAGNOSTICO_EXTRACCION_INVERSORES_MUST.md`.
