...
Novedades de esta versión: 📋 Catálogo de Paneles — corregido falso "multi-modelo" (una ficha de un solo panel real, Suntech STP-410-A72-Pnh-Bifacial, se detectaba como 2 modelos) causado por una nota de auto-verificación cruzada en la misma línea del valor de Pmax (`"410.18 W ✓ coincide con 410.0 Wp"`); el mismo bug también corrompía Isc (0.05 A en vez de 10.49 A) por la misma causa en la línea del coeficiente de temperatura µIsc — confirmado con el PDF real que un solo fix resuelve ambos. Reportado por el usuario, root-caused reproduciendo el texto y luego el PDF real antes de tocar/confirmar el fix. Panel Suntech STP-410-A72-Pnh-Bifacial ingresado al catálogo real. Ver sección 13d.
