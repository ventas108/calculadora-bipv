...
Novedades de esta versión: 📄 Reporte PDF — nueva sección "⚡ Compatibilidad Eléctrica String–Inversor", pedida explícitamente por el usuario tras preguntar la ventaja de integrar, para cada inversor, un gráfico de compatibilidad eléctrica como el que muestra una referencia estándar internacional. Nueva función pura `calculos/dimensionamiento.py::curva_electrica_temperatura()` + gráfico SVG en el reporte (mismo estilo hand-built que las gráficas existentes de Producción/Financiero) que dibuja Voc(T) y Vmp(T) del string contra la ventana MPPT y el límite Vdc máximo del inversor, en las 3 temperaturas de diseño del sitio.
