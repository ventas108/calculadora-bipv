...
Novedades de esta versión: 🔌 Catálogo de Inversores PDF — auditoría real con 5 fichas MUST (PV3500/PV3600/PV3300 TLV Series) pedidas por el usuario. 🔴 **Bug real más serio de esta ronda**: `Vdc_max` confundía un voltaje AC de red ("Max input voltage 270Vac MAX", sección de entrada AC) con el voltaje DC máximo del array FV, porque el patrón genérico no exigía que la etiqueta viniera calificada como PV/DC — el guard de plausibilidad (50-1500V) no lo detectaba porque 270V es un voltaje DC creíble; el error era de origen (AC vs DC), no de rango. Riesgo genérico, no exclusivo de MUST, ya que `Vdc_max` alimenta el gate de seguridad eléctrica (Voc_frío ≤ Vdc_max) de toda la app. Corregido excluyendo explícitamente cualquier coincidencia seguida de "ac"/"AC". Además: hallazgo crítico de un archivo del escritorio mal nombrado (nombrado "PV35-12048 TLV" pero contenía en realidad la serie PV3600, MPPT muy distinto) — no un bug de código; ya renombrado. Y 2 bugs más de código: `P_dc_max_W` no se extraía por un typo real del fabricante ("Maximim" en vez de "Maximum"), y el campo `modelo` confundía el encabezado de sección "INVERTER" con un código de modelo real (fix general, no solo MUST). Primer test directo del motor de extracción de inversores (`tests/test_pdf_inversor_extractor.py`, 7 tests) — antes no tenía ninguno. Suite completa: **768/768**. Ver sección 13b y `DIAGNOSTICO_EXTRACCION_INVERSORES_MUST.md`.
