...
Novedades de esta versión: 🔌 Catálogo de Inversores PDF — auditoría real con fichas MUST (PV3500/PV3600/PV3300 TLV Series) pedidas por el usuario, 5 bugs reales de código encontrados y corregidos. 🔴 Más serios: (1) `Vdc_max` confundía un voltaje AC de red ("Max input voltage 270Vac MAX") con el voltaje DC máximo del array FV — riesgo genérico, no exclusivo de MUST, ya que `Vdc_max` alimenta el gate de seguridad eléctrica de toda la app; corregido con un guard que excluye coincidencias seguidas de "ac"/"AC". (2) Ese mismo guard se podía esquivar por backtracking del propio motor de regex (Python retrocedía un dígito y reintentaba contra el sobrante, truncando 270→27 en vez de rechazar) — corregido con un grupo atómico `(?>...)` que impide el retroceso. Además: hallazgo crítico de un archivo del escritorio mal nombrado (nombrado "PV35-12048 TLV" pero contenía en realidad la serie PV3600) — ya renombrado; `P_dc_max_W` no se extraía por un typo real del fabricante ("Maximim" en vez de "Maximum"); el campo `modelo` confundía el encabezado "INVERTER" con un código de modelo; y un fraseo distinto de Vdc_max ("Maximum Solar Input Voltage" con tolerancia "±N") que ninguna otra ficha MUST usa. Se generó y entregó al usuario una ficha PDF aislada (2 submodelos limpios) para verificar PV33-5048/6048, ya que el detector multi-modelo de la ficha completa (11 modelos) produce entradas basura — hallazgo documentado y mitigado, no corregido de raíz (riesgo de afectar otros fabricantes que dependen de la misma ruta de código). Primer test directo del motor de extracción de inversores (`tests/test_pdf_inversor_extractor.py`, 9 tests) — antes no tenía ninguno. Suite completa: **770/770**. Ver sección 13b y `DIAGNOSTICO_EXTRACCION_INVERSORES_MUST.md`.
