# -*- coding: utf-8 -*-
"""
Script de uso unico: agrega paneles reales de Jinko Solar al catalogo real
(datos/paneles_catalogo.xlsx::Catalogo_Paneles_FV) -- ver
DIAGNOSTICO_CATALOGO_JINKO_NREL.md para el detalle completo.

Mismo patron que agregar_paneles_trina_nrel.py. Cruce por nombre
NORMALIZADO (sin puntuacion) entre las 2 fuentes reales, mismo umbral de
auditoria fisica real (6%, validar_sdm_vs_ficha).

Patron real encontrado en el 13.2% que falla la tolerancia (62/470,
proporcion mayor que Trina): modulos half-cut con sufijo "H" (ej.
JKM335M-72H) -- el paper tradujo cells_in_series=144 (conteo fisico de
medias-celdas) en vez de la profundidad electrica real (72, arquitectura
de 2 strings en paralelo). Verificado con un caso real: CEC y el paper
coinciden en N_s=144 para JKM335M-72H, pero 46,9V/144=0,33V por celda es
fisicamente implausible (una celda real de silicio da ~0,6-0,7V); 46,9V/72
=0,65V si es real -- confirma que la profundidad electrica real es 72, no
144. Mismo mecanismo que el sufijo "(II)" de Trina, aplicado a la linea
half-cut de Jinko (una porcion mayor de su catalogo, de ahi la tasa mas
alta). El chequeo de 6% lo atrapa sin necesidad de diagnosticar cada caso.

Fuentes (mismas 2 que JA Solar/Trina, ver esos scripts para las URLs):
...
Auditoria real: de 470 candidatos con match normalizado, 62 fallan la
tolerancia real de produccion (6%) -- EXCLUIDOS. Quedan 408 candidatos.

DECISION DE DISENO (igual que JA Solar/Trina): no se inyectan I_L_ref/
I_o_ref/R_s/R_sh_ref -- el catalogo Excel nunca los guarda, el Motor IV
los recalcula on-demand con estimar_sdm_desde_ficha(). El unico dato del
paper que si pasa al catalogo es gamma_ref, como "n (Factor Idealidad)".

Uso:
    cd bipv_python
    python datos/agregar_paneles_jinko_nrel.py
"""
import csv
import re
import sys
...
def _construir_pares_jinko(pvs, cec):
    """Cruce normalizado, solo Jinko Solar, primer match por clave
    normalizada."""
    pvs_norm = {}
    for n in pvs:
    ...
    return [
        ...
        for k in comunes
        if "jinko" in cec[cec_norm[k]]["Manufacturer"].lower()
    ]
...
def _construir_fila(nombre_cec, pv, c):
    ...
    modelo = nombre_cec.replace("Jinko Solar Co Ltd ", "").strip()
    ...
    partes.append(
        ...
        "real (ver DIAGNOSTICO_CATALOGO_JINKO_NREL.md)."
    )

    return {
        "ID": None, "Marca": "Jinko Solar", "TipoPanel": modelo,
...
def main():
    ...
    pares = _construir_pares_jinko(pvs, cec)
    print(f"Candidatos Jinko (match normalizado): {len(pares)}")
