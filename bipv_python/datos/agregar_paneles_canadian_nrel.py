# -*- coding: utf-8 -*-
"""
Script de uso unico: agrega paneles reales de Canadian Solar al catalogo
real (datos/paneles_catalogo.xlsx::Catalogo_Paneles_FV) -- ver
DIAGNOSTICO_CATALOGO_CANADIAN_NREL.md para el detalle completo.

Mismo patron que agregar_paneles_jinko_nrel.py. Cruce por nombre
NORMALIZADO (sin puntuacion) entre las 2 fuentes reales, mismo umbral de
auditoria fisica real (6%, validar_sdm_vs_ficha).

NOTA REAL: el nombre legal del fabricante que usan ambas fuentes es "CSI
Solar Co Ltd" -- es el brazo manufacturero real de Canadian Solar Inc.
(confirmado por investigacion de mercado, 3-sep-2026). Una busqueda
literal por "Canadian Solar" en las 2 fuentes no encuentra nada -- hay
que filtrar por "CSI Solar Co Ltd".

Auditoria real: de 384 candidatos con match normalizado, solo 4 fallan la
tolerancia real de produccion (6%) -- la familia CS6X-3XXP, sobrepasos
menores (Vmp/Pmax 6-7%), SIN el patron de half-cut visto en Trina/Jinko --
simple caso limite de ajuste, no necesito diagnostico adicional. Quedan
380 candidatos. Cobertura de dimensiones mas baja que los lotes
anteriores: 310/384 (81%) sin Length/Width en CEC, solo area total --
verificado que es un hueco real de la fuente para este fabricante, no un
error de este script.

Fuentes (mismas 2 que JA Solar/Trina/Jinko, ver esos scripts para URLs):
...
Motor IV los recalcula on-demand con estimar_sdm_desde_ficha(). El unico
dato del paper que si pasa al catalogo es gamma_ref, como "n (Factor
Idealidad)".

Uso:
    cd bipv_python
    python datos/agregar_paneles_canadian_nrel.py
"""
import csv
import re
import sys
...
def _construir_pares_canadian(pvs, cec):
    """Cruce normalizado, solo Canadian Solar (nombre legal real en ambas
    fuentes: "CSI Solar Co Ltd"), primer match por clave normalizada."""
    pvs_norm = {}
    for n in pvs:
    ...
    return [
        ...
        for k in comunes
        if cec[cec_norm[k]]["Manufacturer"].strip().lower() == "csi solar co ltd"
    ]
...
def _construir_fila(nombre_cec, pv, c):
    ...
    modelo = nombre_cec.replace("CSI Solar Co Ltd ", "").strip()
    ...
    partes = [
        ...
        "Fabricante real: Canadian Solar Inc. (marca legal en la fuente: "
        "\"CSI Solar Co Ltd\").",
    ]
    if not tiene_dim:
        partes.append(
    ...
    partes.append(
        ...
        "real (ver DIAGNOSTICO_CATALOGO_CANADIAN_NREL.md)."
    )

    return {
        "ID": None, "Marca": "Canadian Solar", "TipoPanel": modelo,
...
def main():
    ...
    pares = _construir_pares_canadian(pvs, cec)
    print(f"Candidatos Canadian Solar (match normalizado): {len(pares)}")
